import { ProbabilityConfigHandler } from "../source/config/ProbabilityConfigHandler";
...
describe ("Probability Config Reader tests", () => {
    it("given the test config file, it generates the correct configuration for RoomConnectionBehavior", () => {
        let probabilityConfigHandler: ProbabilityConfigHandler = new ProbabilityConfigHandler(
                "../../../config/TESTS/testRoomConnectionBehavior.json"),
            config = probabilityConfigHandler.probabilityMix;
        ...
        expect(config["BLOB_MIX"]["LARGE_BLOBS"].min).to.equal(80);
    ...
    it("given the test config file, it returns the expected values for given mixes and probability", () => {
        let probabilityConfigHandler: ProbabilityConfigHandler = new ProbabilityConfigHandler(
                "../../../config/TESTS/testRoomConnectionBehavior.json");
        debugger;
        expect(probabilityConfigHandler.probabilityAndMixToValue("EVEN_MIX", 21)).to.equal("HALLWAYS");
